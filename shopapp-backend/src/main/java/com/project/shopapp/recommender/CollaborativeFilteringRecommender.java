...
public class CollaborativeFilteringRecommender {
    ...
    public CollaborativeFilteringRecommender(UserRepository userRepository, ProductRepository productRepository, UserRatingRepository userRatingRepository) {
    ...
    public List<Product> recommendedBooks(Long userId) {
        ...
        userRepository.findAll().forEach(userItem -> {
            ...
            userRatingRepository.findAllByUserId(userID).forEach(userBookRating -> {
                        if (userBookRating.getUser().getId().compareTo(userID) == 0) {
                            userRatings.put(userBookRating.getProduct().getId(), userBookRating.getRate());
                        }
                    }
            );
        ...
        productRepository.findAll().forEach(book -> books.put(book.getId(), book.getName()));
        ...
        List<Product> recommendedBooks = new ArrayList<>();
        ...
        while (sortedREntries.hasNext() && i < NUM_RECOMMENDATIONS) {
            ...
            if (entry.getValue() >= MIN_VALUE_RECOMMENDATION) {
                Optional<Product> optionalBook = productRepository.findById(entry.getKey());
    ...
    private Map<Long, Double> getNeighbourhoods(Map<Long, Integer> userRatings) {
        ...
        log.info("CollaborativeFilteringRecommender: getNeighbourhoods() ended" + output);
        return output;
    }
